	...
	*/
    ...
    public function countPage(){
        ...
        if($variance>$this->thre){
            ...
            DB::table('pageanalysis')->insert(array('target_url'=>$this->url,
                'char_division_aTag'=>json_encode($div_aTags_num),
                'variance'=>$variance,
                'return_index'=>json_encode($index)
            ));
            if(!empty($index)){
     ...
     * @return string\
     */
    public function splitSentence($text){
        echo $text;
        ...
        $pa->StartAnalysis();
        //返回文本的关键词，允许的最大个数为10
        $words=$pa->GetFinallyKeywords(10);
        return $words;
    }
    ...
    private function getMainContent($array){
        ...
        $selectedKey=array();
        //寻找当前元素在原始数组中对应的索引
        for($i=0;$i<count($copy_var);$i++){
            if(count($selectedKey)<2&&$copy_var[$i]>0){
